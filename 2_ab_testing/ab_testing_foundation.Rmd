...
-   test if intervention has an effect on target overall eval criteria
    (i.e. does changing the onboarding flow improve early product
    engagement)
...
-   **randomization mechanism** works as expected (if an experiment is
    ...
    are not statistically different from 50/50; sample ratio mismatch
    check)
...
### What to AB test vs not test

-   what to test and not test depends on maturity of product and
...
### Why is randomization so important in AB tests

-   NEXT STEP BOOKMARK
-   Randomly assigning experiment units to experiment buckets help
    ensure that factors outside the experiment intervention are evenly
    spread across the groups which helps with the following:
    -   reduces selection bias

    -   reduces bias from known and known confounding variables (e.g.
        making it less likely that factors outside the intervention
        unevenly influence one of the groups)

    -   reduces uneven time related bias as groups are exposed to same
        time period

    -   randomization used as foundation for statistical inference
        methods where we infer about population using sample

    -   randomization allows us to frame a null hypothesis of no
        difference between groups because the randomization attempts to
        control for factors outside of the experiment intervention

### How to pick **experiment success evaluation criteria**

-   Measurable in the short term (ideally 1 to 2 weeks) and believed to
    causally drive long term strategic metric
-   When north star metric is lagging (y1 renewal) surrogate metrics
    used for experimentation (time to action, capped revenue per user,
    etc)
-   OEC includes success metric and counter metrics (revenue per user
    and user engagement metric)
-   Are the key success metrics sensitive enough to experiment
    on/movable (i.e. month 1 engagement rate at MC is near a ceiling so
    it's not)
-   Measurable in the short-term, sensitive, and predictive of long term
    objectives, and not easily gameable

### Calling experiments using **success evaluation criteria**

-   SHIP: 1 positive, others flat
-   DON'T SHIP: 1 negative, others flat
-   FAIL FAST or PIVOT: all flat, check power
-   DISCUSS TRADEOFFS: some positive and some negative

### Guardrail metric

-   Example: app crash rate
-   MC launches iOS app feature to reduce friction to continue consuming
    previously consumed content
-   However, app crash rate spiked after release of the feature
-   Experiment ended early to resolve issue and relaunch
-   Foundation user experience always has to come first

### Iterative experiments vs full site redesign

-   **Full site redesign experiments**: useful if site is in bad shape
    and not meeting user needs, draw back is users learn to use legacy
    experience and might take time to learn new experience, can be
    costly on time to execute and dev resources for large overhaul, due
    to a set of changes bundled together it will be difficult to
    untangle which of the changes are driving metrics or not, high risk
    due to assumption that new redesign will be better but it is unknown
    in advance
-   **Iterative experiments**: lower risk and faster to execute, test
    one feature/theme at a time to attribute effect to intervention,
    users have less UX shock as the entire product UX is not changing
    drastically, iterative experiments can result in always learning
    feedback loops

### Hypothesis testing as part of AB testing

-   Objective

    -   test if the difference between control and treatment is unlikely
        given null hypothesis assumption that differences are zero

-   Test setup

    -   frame null and alt hypothesis

    -   predefined alpha (significance level) level threshold: if pvalue
        below alpha we conclude stat sig difference between groups

    -   practical significance level: for lift to be meaningful for the
        business it should be greater than or equal to X

    -   power analysis (power 0.8) to derive predetermine sample size:
        significance 0.95 and MDE needed as inputs

-   Run test

    -   collect data / run test till predetermine sample size is reached

    -   recommended to run test for at least 1 week to capture day of
        week and weekend users/patterns

-   Analyze

    -   generate test stat: group difference / std error

    -   generate pvalue using test stat

    -   if pvalue is below alpha level then we have evidence to reject
...
-   Run power analysis for deriving sample size target based on exp
    inputs
-   Yypically recommended to run exp for at least one business
