...
-   if an experiment spikes time to value or causes the app to crash it
    ...
    rollout even when primary metric(s) are positive
...
-   **Full site/app redesign experiments**
    ...
    -   can be costly on time to execute and dev resources for large
        overhaul
    -   due to a set of changes bundled together it will be difficult to
        untangle which of the changes are driving metrics or not
...
-   NEXT STEP BOOKMARK (improvement collaboration with GPT:
    <https://chatgpt.com/c/86c48943-23d0-4f17-982f-9a50380afcfd>)
...
-   Objective
    ...
    -   we typically test if the difference between control and
        treatment is unlikely given null hypothesis assumption that
...
-   Analyze
    ...
    -   generate test statistic from experiment sample data: group
